...
class Redactor:
    ...
    def __init__(self, filename, subdir, savesubdir):
        ...
        self.delimiters = random.choice([' ', ', ', '; ', ' \n '])
        self.fakerdict = {
            'A2': self.delimiters.join([self.fake.name(), self.fake.address(), self.fake.phone_number(), self.fake.free_email()]),
            'D2': self.fake.name(),
            'D3': self.fake.company(),
            'D4': self.fake.address(),
            'D5': self.delimiters.join([self.fake.phone_number(), self.fake.free_email()]),
        }
        ...
        if not A:
            if self.lang == "fr":
                for joiner in joiners:
                    A = p.search(question_no + joiner +
                                 unidecode(self.labeldict[key][self.lang]))
                    if A:
                        break
        if not A:
            for joiner in joiners:
                A = p.search(question_no + joiner +
                             self.labeldict[key][self.lang])
# if	key	== "footer":
        ...
        for page in doc:
            ...
            for lang in ["en", "fr"]:
                if page_ocr.search("A. "+self.labeldict["A"][lang]):
                    self.lang = lang
                    X_search = page_ocr.search(
                        "A. " + self.labeldict["A"][lang])[0][0][0]
                    Y_search = page_ocr.search(
                        "A. " + self.labeldict["A"][lang])[0][0][1]
...
# print(YFactor,	YFactor_small)
                ...
                try:
                    ...
                    if FOOTER:
                        ...
                        print(errmsg)
                        print(
                            f"scaling	from B4_Y0:{B4_Y0} by ({A2_Y1} - {A2_Y0}) *	2")
...
# text_A2	= self.get_pmaptext(area_A2, page)
                    text_A2 = self.fakerdict['A2']
# print("text_A2", text_A2)
#                    text_B4 = self.get_pmaptext(area_B4, page)
                    text_B4 = self.fake.paragraph(nb_sentences=4)
...
# self.errorlog = self.errorlog + ' | ' + errmsg
                    ...
                    areas_page2 = [area_D2,	area_D3, area_D4,
                                   area_D5, area_E1,	area_E2, area_E3]
#                    text_D2 = self.get_pmaptext(area_D2, page)
                    text_D2 = self.fakerdict['D2']
#                    text_D3 = self.get_pmaptext(area_D3, page)
                    text_D3 = self.fakerdict['D3']
#                    text_D4 = self.get_pmaptext(area_D4, page)
                    text_D4 = self.fakerdict['D4']
